...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var ffmpegAc3Decoder: FFmpegAc3Decoder
    private var currentMediaProbeResult: FFmpegAc3Decoder.MediaProbeResult? = null
    ...
    private fun initServices() {
        ...
        ffmpegAc3Decoder = FFmpegAc3Decoder(this)
    ...
    private fun showAudioTrackDialog() {
        ...
        val probe = currentMediaProbeResult
        val audioOptions = mutableListOf<Triple<Int, String, Boolean>>() // (trackIndex, displayName, isAc3)

        if (probe != null && probe.audioStreams.isNotEmpty()) {
            for (stream in probe.audioStreams) {
                val label = if (stream.isAc3) {
                    "${stream.title} • FFmpeg SW Decoder"
                } else {
                    stream.title
                }
                audioOptions.add(Triple(stream.index, label, stream.isAc3))
            }
        } else {
            try {
                val trackInfo = mp.trackInfo
                ...
                for (i in trackInfo.indices) {
                    if (trackInfo[i].trackType == MediaPlayer.TrackInfo.MEDIA_TRACK_TYPE_AUDIO) {
                        ...
                        audioOptions.add(Triple(i, lang, false))
                        audioCounter++
                    }
                }
            } catch (e: Exception) {
                Log.w(PtpConstants.TAG, "Error reading MediaPlayer trackInfo", e)
            }
        }
        ...
        val names = audioOptions.map { it.second }.toTypedArray()
        AlertDialog.Builder(this)
            .setTitle(getString(R.string.audio_track))
            .setItems(names) { dialog, which ->
                val selected = audioOptions[which]
                val currentSession = videoPlaybackManager.getCurrentSession()
                if (selected.third && currentSession != null) {
                    // AC3 audio stream selected: decode via FFmpegKit software decoder
                    val sessionId = currentVideoSessionId
                    layoutVideoBuffering.visibility = View.VISIBLE
                    tvBuffering.text = "Decoding AC3 audio with FFmpeg..."
                    btnVideoAudioTrack.text = "🔊 ${selected.second}"

                    lifecycleScope.launch {
                        val decodedAudio = ffmpegAc3Decoder.decodeAudioStream(
                            inputPath = currentSession.cacheFileLocation.absolutePath,
                            audioStreamIndex = selected.first,
                            sessionId = sessionId
                        )
                        if (sessionId == currentVideoSessionId) {
                            layoutVideoBuffering.visibility = View.GONE
                            if (decodedAudio != null && decodedAudio.exists()) {
                                videoView.setCompanionAudio(decodedAudio)
                                Toast.makeText(this@MainActivity, "Playing AC3 track with FFmpeg software decoder", Toast.LENGTH_SHORT).show()
                            } else {
                                Toast.makeText(this@MainActivity, "FFmpeg AC3 decoding failed, using native audio", Toast.LENGTH_SHORT).show()
                            }
                        }
                    }
                } else {
                    try {
                        videoView.clearCompanionAudio()
    ...
    private fun startVideoPlayback(item: PtpMediaItem) {
        ...
        videoPlaybackManager.closeCurrentSession()
        ffmpegAc3Decoder.cancel()
        ffmpegAc3Decoder.cleanupTempFiles(keepSessionId = sessionId)
        currentMediaProbeResult = null
        try {
        ...
        videoView.setOnErrorListener { _, what, extra ->
            ...
            // Check if this error is due to AC3 or unsupported codec; attempt FFmpegKit software decode
            val probe = currentMediaProbeResult
            if (probe != null && probe.hasAc3) {
                layoutVideoBuffering.visibility = View.VISIBLE
                tvBuffering.text = "AC3 audio detected. Decoding with FFmpeg..."
                tvVideoError.visibility = View.GONE

                lifecycleScope.launch {
                    val remuxFile = ffmpegAc3Decoder.remuxVideoWithAac(
                        inputPath = session.cacheFileLocation.absolutePath,
                        sessionId = sessionId
                    )
                    if (sessionId == currentVideoSessionId) {
                        if (remuxFile != null && remuxFile.exists()) {
                            tvVideoBadgeFormat.text = "${item.filename.substringAfterLast('.', "VIDEO").uppercase()} • AC3 (FFmpeg SW)"
                            videoView.setDataSource(remuxFile.absolutePath)
                        } else {
                            layoutVideoBuffering.visibility = View.GONE
        ...
        lifecycleScope.launch {
            ...
            if (sessionId == currentVideoSessionId) {
                ...
                // Asynchronously probe for AC3 audio streams using FFprobeKit
                launch(Dispatchers.IO) {
                    try {
                        val probeResult = ffmpegAc3Decoder.probeMedia(session.cacheFileLocation.absolutePath)
                        if (sessionId == currentVideoSessionId) {
                            currentMediaProbeResult = probeResult
                            if (probeResult.hasAc3) {
                                withContext(Dispatchers.Main) {
                                    if (sessionId == currentVideoSessionId) {
                                        tvVideoBadgeFormat.text = "${item.filename.substringAfterLast('.', "VIDEO").uppercase()} • AC3 (FFmpeg)"
                                    }
                                }
                            }
                        }
                    } catch (e: Exception) {
                        Log.w(PtpConstants.TAG, "FFmpeg probe failed", e)
                    }
                }
            }
        }
    ...
    private fun stopAndClearVideoPlayer() {
        ...
        videoPlaybackManager.closeCurrentSession()
        ffmpegAc3Decoder.cancel()
        ffmpegAc3Decoder.cleanupTempFiles()
        currentMediaProbeResult = null
        try {
            videoView.setOnPreparedListener(null)
            videoView.setOnErrorListener(null)
